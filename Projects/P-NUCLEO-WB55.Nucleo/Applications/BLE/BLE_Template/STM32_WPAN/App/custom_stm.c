...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomMy_CsHdle;                    /**< My_Custom_Service handle */
  uint16_t  CustomGpio_CHdle;                  /**< My_GPIO_Char handle */
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomGpio_CHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_GPIO_C_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_GPIO_C_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for My_Custom_Service +
   *                                2 for My_GPIO_Char +
   *                                1 for My_GPIO_Char configuration descriptor +
   *                              = 4
   *
  ...
  ret = aci_gatt_add_service(UUID_TYPE_16,
                             ...
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomMy_CsHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: My_CS, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_service command: My_CS \n\r");
  }
   ...
   */
  uuid.Char_UUID_16 = 0x0000;
  ret = aci_gatt_add_char(CustomContext.CustomMy_CsHdle,
                          UUID_TYPE_16, &uuid,
                          SizeGpio_C,
                          ...
                          &(CustomContext.CustomGpio_CHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : GPIO_C, error code: 0x%x \n\r", ret);
  }
  else
  {
    APP_DBG_MSG("  Success: aci_gatt_add_char command   : GPIO_C \n\r");
  }
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_GPIO_C:
      ret = aci_gatt_update_char_value(CustomContext.CustomMy_CsHdle,
                                       CustomContext.CustomGpio_CHdle,
                                       0, /* charValOffset */
                                       SizeGpio_C, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value GPIO_C command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value GPIO_C command\n\r");
      }
